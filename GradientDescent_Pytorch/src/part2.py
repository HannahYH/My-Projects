...
class LinearModel:
    ...
    def activation(self, x):
        ...
        """
        t = type(x)
        if t == bool or t == list or t == np.ndarray:
          raise ValueError
        return 1 / (1 + np.exp(-x))
    ...
    def forward(self, inputs):
        ...
        """
        one = np.ones(1)
        inputs = np.concatenate((inputs, one), axis=0)
    ...
    def loss(prediction, label):
        ...
        """
        return - (np.log(prediction) * label + np.log(1-prediction) * (1-label))
    ...
    def backward(self, inputs, diff):
        ...
        """
        one = np.ones(1)
        self.weights += self.lr * np.concatenate((inputs, one), axis=0) * diff
